...
We define a context-only baseline (same architecture with \texttt{memory\_enabled=False}---the UNet conditions only on action embeddings and noise level, without memory tokens). Both models are trained with identical hyperparameters, data, and random seeds for fair comparison. Our primary evaluation focuses on the full MemGameNGen model.
